...
- samplePath: [required] A file path to where the associated sample is located in your file system; this should end with the file extension `.bcf`, `.vcf`, or `.vcf.gz` if working with array data (ex: `/path/to/sample/array_one.vcf.gz`) or `.bam` if working with lcWGS data (ex: `/path/to/sample_1.bam`)
