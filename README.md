...
While documentation and tests are being developed for low pass WGS inputs, the pipeline is currently unavailable for LPWGS. Note that if needing to run both array and lcWGS data, these runs will need to be performed in separate project directories.
...
- samplePath: [required] A file path to where the associated sample is located in your file system; when working with arrays, this should end with the file extension `.bcf`, `.vcf`, or `.vcf.gz` (ex: `/path/to/sample/array_one.vcf.gz`) or `.bam`/`.cram` if working with lcWGS data (ex: `/path/to/sample_1.bam`)
- sampleIndex: [required] A file path to where the associated sample's indexed file is located in your file system; this should end with the file extension `.csi` (ex: `/path/to/sample/array_one.vcf.gz.csi`)
- pedigree [optional] An optional file path to where the associated sample's pedigree information is located in your file system; this should end with the file extension `.fam`, `.txt`, or `.tsv` (ex: `/path/to/sample/pedigree_array_one.fam`) and contain one line per sample having parent(s) in the dataset and three tab-delimited columns containing kidID fatherID motherID. See below and the SHAPEIT5 documentation for more detail. Note that if your input dataset is lcWGS data, this column should remain empty as GLIMPSE2 does not currently support phasing to pedigree information

For array data: each row in the samplesheet metadata should represent a group of samples that need to be imputed - that is, to be specific, not a single file per individual.

For lcWGS data: each row in the samplesheet metadata should represent a SINGLE INDIVIDUAL that needs to be imputed - that is, to be specific, a single file per individual.
