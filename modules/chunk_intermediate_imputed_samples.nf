 ...
 */
 ...
 process chunk_samples {
    ...
    publishDir(
        path:    "${params.publishDirData}/.twostep_chunked_regions/",
    ...
    output:
        tuple val(sample_id), path(sampleBcf), path(sampleBcfIndex), val(chromosomeNum), path("${sampleBcf.baseName}_${chromosomeNum}_chunked_coords.txt"), path(recombinationMapFile), emit: twoChunkedRegions
