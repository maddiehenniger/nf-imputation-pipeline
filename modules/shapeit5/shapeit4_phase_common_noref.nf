 ...
 */
 ...
 process shapeit4_phase_common_noref {
    ...
    output:
        tuple val(chromosome), val(sMetadata), path("${sMetadata.sampleID}.${chromosome}.phased.bcf"), path(wgs), path(wgsIndex), val(rMetadata), path(reference), path(referenceIndices), path(geneticMap), emit: phasedSamples
